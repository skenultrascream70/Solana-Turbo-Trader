...
import { TurboTransactionExecutor } from './transactions/turbo-transaction-executor';
...
function printDetails(wallet: Keypair, quoteToken: Token, bot: Bot) {
  ...
  logger.info(
    `Using ${TRANSACTION_EXECUTOR} executer: ${bot.isTurbo || bot.isJito || (TRANSACTION_EXECUTOR === 'default' ? true : false)}`,
  );
  if (bot.isTurbo || bot.isJito) {
...
const runListener = async () => {
  ...
  switch (TRANSACTION_EXECUTOR) {
    case 'turbo': {
      txExecutor = new TurboTransactionExecutor(CUSTOM_FEE);
      break;
    }
    case 'jito': {
